...
app.get('/', async (req, res) => {
  res.status(200).send({
    message: 'Hello from Healper!!'
  })
})

app.post('/', async (req, res) => {
  try {
    const prompt = "Give me 10 remedies for " + req.body.prompt;
    ...
    res.status(200).send({
      bot: "Here are 10 remedies for " + req.body.prompt + response.data.choices[0].text
    });
